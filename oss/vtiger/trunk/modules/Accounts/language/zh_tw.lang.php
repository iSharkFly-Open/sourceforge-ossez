<?php
/**
 * Copyright (C) 2006-2010 YUCHENG HU
 *
 * ---------------------------------------------
 * HA WEBSYSTEMS
 * http://www.hawebs.net
 * https://www.hawebs.org/forums/computer/
 *
 * CONTACT
 * huyuchengus@gmail.com / yuchenghu@hawebs.net
 * 
 * ---------------------------------------------
 * [A] GNU GENERAL PUBLIC LICENSE GNU/LGPL
 * [B] Apache License, Version 2.0
 *
 * ---------------------------------------------
 * NOTE
 * 1. 所有的語言配置文件請采用 UTF-8 編碼 
 *
 * ---------------------------------------------
 */
$mod_strings=Array(
'LBL_MODULE_NAME'=>'帳戶',
'LBL_MODULE_TITLE'=>'帳戶：首頁',
'LBL_SEARCH_FORM_TITLE'=>'搜尋帳戶',
'LBL_LIST_FORM_TITLE'=>'帳戶列表',
'LBL_NEW_FORM_TITLE'=>'新增帳戶',
'LBL_MEMBER_ORG_FORM_TITLE'=>'成員組織',
// Label for Top Accounts in Home Page, added for 4.2 GA
'LBL_TOP_ACCOUNTS'=>'我的重要帳戶',
'LBL_TOP_AMOUNT'=>'金額',
'LBL_LIST_ACCOUNT_NAME'=>'帳戶名稱',
'LBL_LIST_CITY'=>'城市',
'LBL_LIST_WEBSITE'=>'網站',
'LBL_LIST_STATE'=>'州',
'LBL_LIST_PHONE'=>'電話',
'LBL_LIST_EMAIL_ADDRESS'=>'電子郵件地址',
'LBL_LIST_CONTACT_NAME'=>'聯系人姓名',
'LBL_LIST_AMOUNT'=>'合共總計',

//DON'T CONVERT THESE THEY ARE MAPPINGS
'db_name'=>'LBL_LIST_ACCOUNT_NAME',
'db_website'=>'LBL_LIST_WEBSITE',
'db_billing_address_city'=>'LBL_LIST_CITY',

//END DON'T CONVERT

'LBL_ACCOUNT'=>'客戶：',
'LBL_ACCOUNT_NAME'=>'客戶名稱：',
'LBL_PHONE'=>'電話：',
'LBL_WEBSITE'=>'網站：',
'LBL_FAX'=>'傳真：',
'LBL_TICKER_SYMBOL'=>'票符號：',
'LBL_OTHER_PHONE'=>'其它電話：',
'LBL_ANY_PHONE'=>'任何電話：',
'LBL_MEMBER_OF'=>'成員：',
'LBL_EMAIL'=>'電子郵件：',
'LBL_EMPLOYEES'=>'員工：',
'LBL_OTHER_EMAIL_ADDRESS'=>'其它電子郵件：',
'LBL_ANY_EMAIL'=>'任何電子郵件：',
'LBL_OWNERSHIP'=>'所有者：',
'LBL_RATING'=>'評價：',
'LBL_INDUSTRY'=>'行業：',
'LBL_SIC_CODE'=>'統一編號：',
'LBL_TYPE'=>'類型：',
'LBL_ANNUAL_REVENUE'=>'全年收入：',
'LBL_ADDRESS_INFORMATION'=>'地址資料',
'LBL_ACCOUNT_INFORMATION'=>'賬號資料',
'LBL_CUSTOM_INFORMATION'=>'客戶資料',
'LBL_BILLING_ADDRESS'=>'賬單地址：',
'LBL_SHIPPING_ADDRESS'=>'送貨地址：',
'LBL_ANY_ADDRESS'=>'其它地址：',
'LBL_CITY'=>'城市：',
'LBL_STATE'=>'州：',
'LBL_POSTAL_CODE'=>'郵政編碼：',
'LBL_COUNTRY'=>'國家：',
'LBL_DESCRIPTION_INFORMATION'=>'描述資料',
'LBL_DESCRIPTION'=>'描述：',
'NTC_COPY_BILLING_ADDRESS'=>'復制賬單地址內容到送貨地址',
'NTC_COPY_SHIPPING_ADDRESS'=>'復制送貨地址內容到賬單地址',
'NTC_REMOVE_MEMBER_ORG_CONFIRMATION'=>'您確定要刪除這筆成員/組織的記錄嗎？',
'LBL_DUPLICATE'=>'復制潛在客戶',
'MSG_DUPLICATE'=>'由潛在客戶中復制建立新客戶，您可以點選客戶列表中的客戶以復制建立新的客戶，并且繼承它的相關數據。',

'LBL_INVITEE'=>'聯絡人',
'ERR_DELETE_RECORD'=>'要刪除賬號您必須指定一筆記錄編號.',

'LBL_SELECT_ACCOUNT'=>'選擇帳戶',
'LBL_GENERAL_INFORMATION'=>'主要信息',

//for v4 release added
'LBL_NEW_POTENTIAL'=>'新增潛在案件',
'LBL_POTENTIAL_TITLE'=>'潛在案件',

'LBL_NEW_TASK'=>'新增任務',
'LBL_TASK_TITLE'=>'任務',
'LBL_NEW_CALL'=>'新增電話記錄',
'LBL_CALL_TITLE'=>'電話記錄',
'LBL_NEW_MEETING'=>'新增會議記錄',
'LBL_MEETING_TITLE'=>'會議',
'LBL_NEW_EMAIL'=>'新增電子郵件',
'LBL_EMAIL_TITLE'=>'郵件',
'LBL_NEW_CONTACT'=>'新增聯絡人',
'LBL_CONTACT_TITLE'=>'聯絡人',

//Added vtiger_fields after RC1 - Release
'LBL_ALL'=>'全部',
'LBL_PROSPECT'=>'前景',
'LBL_INVESTOR'=>'投資者',
'LBL_RESELLER'=>'經銷商',
'LBL_PARTNER'=>'合作夥伴',

// Added for 4GA
'LBL_TOOL_FORM_TITLE'=>'帳戶工具',
//Addedfor4GA
'AccountName'=>'帳戶名稱',
'Phone'=>'電話',
'Website'=>'網址',
'Fax'=>'傳真',
'TickerSymbol'=>'傳票符號',
'OtherPhone'=>'其它電話',
'MemberOf'=>'成員',
'Email'=>'電子郵件',
'Employees'=>'員工',
'OtherEmail'=>'其它電子郵件',
'Ownership'=>'擁有者',
'Rating'=>'評價',
'industry'=>'行業別',
'SICCode'=>'統一編號',
'Type'=>'類型',
'AnnualRevenue'=>'全年收入',
'AssignedTo'=>'負責人',
'BillingAddress'=>'賬單地址',
'ShippingAddress'=>'送貨地址',
'BillingCity'=>'城市地址',
'ShippingCity'=>'送貨城市',
'BillingState'=>'州地址',
'ShippingState'=>'送貨州',
'BillingCode'=>'郵政編碼地址',
'ShippingCode'=>'送貨郵政編碼',
'BillingCountry'=>'國家'地址,
'ShippingCountry'=>'送貨國家',
'CreatedTime'=>'建立時間',
'ModifiedTime'=>'修改時間',
'Description'=>'描述',
'ShippingPoBox'=>'付款地址采購訂單信箱',
'BillingPoBox'=>'付款地址采購訂單信箱',

//Added after 4.2 patch 2
'EmailOptOut'=>'拒絕電子郵件打擾',
'LBL_EMAIL_OPT_OUT'=>'拒絕電子郵件打擾：',

//Added after 5Alpha5
'NotifyOwner'=>'提醒負責人',

//Added for existing picklist entries

'--None--'=>'--無--',

'Acquired'=>'取得',
'Active'=>'啟用',
'MarketFailed'=>'推銷失敗',
'ProjectCancelled'=>'項目取消',
'Shutdown'=>'關閉',

'Apparel'=>'服裝',
'Banking'=>'銀行',
'Biotechnology'=>'生物科技',
'Chemicals'=>'化學',
'Communications'=>'交通業',
'Construction'=>'建築業',
'Consulting'=>'顧問',
'Education'=>'教育業',
'Electronics'=>'電子業',
'Energy'=>'能源',
'Engineering'=>'工程業',
'Entertainment'=>'娛樂事業',
'Environmental'=>'環境業',
'Finance'=>'財務',
'Food&Beverage'=>'飲食',
'Government'=>'政府',
'Healthcare'=>'醫療保健',
'Hospitality'=>'醫院',
'Insurance'=>'保險業',
'Machinery'=>'機械業',
'Manufacturing'=>'製造業',
'Media'=>'媒體行業',
'Not For Profit'=>'非營利機構',
'Recreation'=>'康樂',
'Retail'=>'零售業',
'Shipping'=>'運輸業',
'Technology'=>'資訊科技',
'Telecommunications'=>'通訊業',
'Transportation'=>'交通業',
'Utilities'=>'工具',
'Other'=>'其它',

'Analyst'=>'分析師',
'Competitor'=>'競爭者',
'Customer'=>'客戶',
'Integrator'=>'整合者',
'Investor'=>'投資者',
'Partner'=>'伙伴',
'Press'=>'新聞',
'Prospect'=>'潛在客戶',
'Reseller'=>'經銷商',
'LBL_START_DATE'=>'開始日期',
'LBL_END_DATE'=>'結束日期',

// Added/Updated for vtiger CRM 5.0.4

//added to fix the issue #4081
'LBL_ACCOUNT_EXIST'=>'帳戶名稱已存在!',

// mailer export
'LBL_MAILER_EXPORT'=>'郵寄導出',
'LBL_MAILER_EXPORT_CONTACTS_TYPE'=>'選擇通訊錄:',
'LBL_MAILER_EXPORT_CONTACTS_DESCR'=>'聯絡可能被選擇通過"自定義字段"和一些標準字段。.',
'LBL_MAILER_EXPORT_RESULTS_TYPE'=>'選擇導出類型:',
'LBL_MAILER_EXPORT_RESULTS_DESCR'=>'聚集從賬戶及其聯絡人的數據,還給前次搜索.',
'LBL_EXPORT_RESULTS_EMAIL'=>'導出郵件聯絡人數據',
'LBL_EXPORT_RESULTS_EMAIL_CORP'=>'導出郵件數據,如果聯絡人"電郵"為空則用"公司電郵"',
'LBL_EXPORT_RESULTS_FULL'=>'導出聯絡人數據，電子郵件帳戶名稱，地址，電話等.',
'LBL_EXPORT_RESULTS_GO'=>'導出',
'LBL_MAILER_EXPORT_IGNORE'=>'--忽略--',
'LBL_MAILER_EXPORT_CHECKED'=>'檢查',
'LBL_MAILER_EXPORT_NOTCHECKED'=>'不進行檢查',

// Added after 5.0.4 GA

//Module Sequence Numbering
'AccountNo'=>'帳戶號碼',
//END

// Account Hierarchy
'LBL_SHOW_ACCOUNT_HIERARCHY'=>'查看帳號級別',

);

?>
