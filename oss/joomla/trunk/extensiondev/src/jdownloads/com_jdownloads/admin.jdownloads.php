       ...
       if ($task == 'restore.run'){
        ...
        if ($task == '' or $task == 'scan.files') {
            ...
            if ($new_files > 0){
                ...
                echo '<td><font color="green"><big>'.JText::_('JLIST_BACKEND_AUTOCHECK_NO_NEW_FILES').'</big></font></td>';
            }
            if ($mis_files > 0){
...
function categoriesSave($option,$apply=0){
   ...
   $database->query();    
	...
	if (!$apply) {
		...
		$mainframe->redirect("index2.php?option=".$option."&task=categories.edit&cid=".$row->cat_id, JText::_('JLIST_BACKEND_CATSEDIT_SAVE')." ".$msg);
	} 	
}
...
function categoriesEdit($option, $cid){
  ...
  . " onchange=\"javascript:if (document.forms[0].cat_pic.options[selectedIndex].value!='') {document.imagelib.src='../$cat_pic_dir' + document.forms[0].cat_pic.options[selectedIndex].value} else {document.imagelib.src=''}\"", 'value', 'text', $row->cat_pic );
...
function categoriesList($option, $task, $limitstart){
    ...
    foreach ($tree as $row){
        ...
        $row->sum_files = $database->loadResult();
    }	
...
function filesSave($option, $cat_id, $apply=0){
    ...
    if (empty($row->file_title)) {
        if(!$padfile['tmp_name']){
           $mainframe->redirect("index2.php?option=".$option."&task=files.edit&cid=".$row->file_id, JText::_('JLIST_BACKEND_FILESEDIT_ERROR_TITLE'));
        }
    } else {
    ...
    // fill the alias with a valid value
    if(empty($row->file_alias)) {
       $row->file_alias = $row->file_title;
    }
    $row->file_alias = JFilterOutput::stringURLSafe($row->file_alias);
	...
	if (empty($marked_cat_id)) {
        $mainframe->redirect("index2.php?option=".$option."&task=files.edit&cid=".$row->file_id, JText::_('JLIST_BACKEND_FILESEDIT_CATLIST_ERROR'));
    }  
	...
	if($file['tmp_name'] != ''){
        ...
        if(@move_uploaded_file($file['tmp_name'], $target_path)) {
		   ...
		   $mainframe->redirect("index2.php?option=".$option."&task=files.edit&cid=".$row->file_id, JText::_('JLIST_BACKEND_FILESEDIT_CHECK_PERMISSIONS')." ");
		}
 	} else {
	...
	else $mainframe->redirect("index2.php?option=".$option."&task=files.edit&cid=".$row->file_id."&cat_id=".$cat_id, JText::_('JLIST_BACKEND_FILESEDIT_SAVE').$movedmsg." ");
}
...
function filesEdit($option, $cid, $cat_id){
  ...
  . " onchange=\"javascript:if (document.forms[0].file_pic.options[selectedIndex].value!='') {document.imagelib.src='../$file_pic_dir' + document.forms[0].file_pic.options[selectedIndex].value} else {document.imagelib.src=''}\"", 'value', 'text', $row->file_pic );
...
function showConfig($option){
  ...
  . " onchange=\"javascript:if (document.forms[0].down_pic2.options[selectedIndex].value!='') {document.imagelib9.src='../$down_pic_dir' + document.forms[0].down_pic2.options[selectedIndex].value} else {document.imagelib9.src=''}\"", 'value', 'text', $jlistConfig['download.pic.files'] ); 
  ...
  . " onchange=\"javascript:if (document.forms[0].upd_pic.options[selectedIndex].value!='') {document.imagelib8.src='../$upd_pic_dir' + document.forms[0].upd_pic.options[selectedIndex].value} else {document.imagelib8.src=''}\"", 'value', 'text', $jlistConfig['picname.is.file.updated'] );
	...
	jlist_HTML::showConfig($option, $list_sortorder, $cats_sortorder, $user_groups, $file_plugin_inputbox, $file_plugin_inputbox2, $inputbox_pic, $inputbox_pic_file, $inputbox_hot, $inputbox_new, $inputbox_down, $inputbox_down2, $inputbox_mirror_1, $inputbox_mirror_2, $inputbox_upd);
}
...
function saveConfig($option){
    ...
    if (!is_dir(JPATH_SITE.DS.$jlistConfig['pad.folder'])){
       if ($makedir = @mkdir(JPATH_SITE.DS.$jlistConfig['pad.folder'].DS, 0777)) {
