...
class SEFModelSEFUrls extends JModel
{
    ...
    function export($where = '')
    {
        ...
        $fields = array('cpt', 'sefurl', 'origurl', 'Itemid', 'metadesc', 'metakey', 'metatitle', 'metalang', 'metarobots', 'metagoogle', 'metacustom', 'canonicallink', 'dateadd', 'priority', 'trace', 'enabled', 'locked', 'sef', 'sm_indexed', 'sm_date', 'sm_frequency', 'sm_priority', 'trashed');
    ...
    function UpdateURLs()
    {
        ...
        $query = "UPDATE `#__sefurls` SET `trashed` = '1' WHERE `dateadd` = '0000-00-00' AND `locked` = '0'";
        $db->setQuery($query);
        if (!$db->query()) {
    ...
    /**
     * Prepares the database and configuration for automatic SEF URLs generation
     */
    function prepareGenerate()
    {
        $db =& JFactory::getDBO();
        
        $db->setQuery("UPDATE `#__sefurls` SET `flag` = '0'");
        if (!$db->query()) {
            return false;
        }
        
        // Generate the homepage
        SEFTools::PostRequest(JURI::root(), null, null, 'get');
        
        /*$sefConfig =& SEFConfig::getConfig();
        $sefConfig->disableNewSEF = false;
        */
    ...
    function generate()
    {
        ...
        // Get the URLs not searched yet, but 500 max
        $db->setQuery("SELECT DISTINCT `sefurl` FROM `#__sefurls` WHERE `origurl` != '' AND `flag` = '0' AND `trashed` = '0' LIMIT 50");
        $rows = $db->loadObjectList();
        
        if (count($rows) == 0) {
            // Done
            $db->setQuery("SELECT COUNT(`id`) FROM `#__sefurls` WHERE `trashed` = '0'");
            $count = $db->loadResult();
            return "{ type: 'complete', searched: {$count}, left: 0, total: {$count} }";
        }
        
        // Get time limit (max. 60% of PHP's time limit)
        $timelimit = ini_get('max_execution_time');
        $timelimit = intval($timelimit * 0.6);
        /*
        if ($timelimit > 5) {
            $timelimit = 5;
        }
        */
        ...
        // Loop through URLs and "visit" them
        foreach ($rows as $row) {
            // Create URL
            $url = rtrim(JURI::root(), '/') . '/' . ltrim($row->sefurl, '/');

            // Set as searched
            $db->setQuery("UPDATE `#__sefurls` SET `flag` = '1' WHERE `sefurl` = ".$db->Quote($row->sefurl));
            $db->query();
            ...
            $errno = null;
            $errstr = null;
            $conns[$i] = @fsockopen($host, 80, $errno, $errstr, 1);
            if( $conns[$i] === false ) {
                continue;
            }
            ...
            fputs($conns[$i], "GET {$path}{$query} HTTP/1.1\r\n");
            fputs($conns[$i], "Host: $host\r\n");
            fputs($conns[$i], "Connection: close\r\n\r\n");
            
            // Don't wait for answer, close the socket connection:
            //fclose($fp);
            
            //SEFTools::PostRequest($url, null, null, 'get');
    ...
    function generateStatus()
    {
        ...
        // Get counts
        $db->setQuery("SELECT `flag`, COUNT(`id`) AS `num` FROM `#__sefurls` WHERE `origurl` != '' AND `trashed` = '0' GROUP BY `flag`");
        $counts = $db->loadObjectList('flag');
        
        $searched = isset($counts['1']) ? $counts['1']->num : 0;
        $left = isset($counts['0']) ? $counts['0']->num : 0;
        $total = $searched + $left;
        
        return "{ type: 'status', searched: {$searched}, left: {$left}, total: {$total} }";
    }
}
?>