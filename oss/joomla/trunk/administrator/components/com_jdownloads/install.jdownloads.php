...
function com_install() {
	...
	$frontend_lang = $params->get('site', 'zh-CN');
...
<table width="100%" border="0">
	<tr>
		<td align="center"><img
			src="components/com_jdownloads/images/jdownloads.jpg" border="0"
			alt="jDownloads Logo" /><br />
			<?php echo  'Version '.$jd_version.' '.$jd_version_state; ?></td>
	</tr>
	<tr>
		<td background="E0E0E0" style="border: 1px solid #999;"><code><b><?php echo JText::_('JLIST_INSTALL_0'); ?></b><br />
		...
		if ( !in_array ( $prefix.'jdownloads_config', $tablelist ) ){
			...
			if ($version && $version < 1.4) {
				...
				if ($create_root_ok){
					// dirs l�schen falls nicht verschoben wurde, da schon da
					foreach($source_dir as $source){
			   ...
			   echo "<font color='green'>--> ".JText::_('JLIST_INSTALL_9')."<br />";
				...
				// tempzipfiles
				$dir_existzip = is_dir(JPATH_SITE."/downloads/tempzipfiles");

				if($dir_existzip) {
					if (is_writable(JPATH_SITE."/downloads/tempzipfiles")) {
						echo "<font color='green'>--> ".JText::_('JLIST_INSTALL_11')."</font><br />";
					} else {
						echo "<font color='red'><strong>--> ".JText::_('JLIST_INSTALL_12')."</strong></font><br />";
					}
				} else {
					if ($makedir = @mkdir(JPATH_SITE."/downloads/tempzipfiles/", 0777)) {
				...
				if ($root_dir == ''){
					...
					if($dir_exist) {
						if (is_writable(JPATH_SITE."/downloads")) {
							if (!is_dir(JPATH_SITE."/downloads/".JText::_('JLIST_SAMPLE_DATA_CAT_FOLDER_ROOT'))){
								// daten speichern
								// dirs f�r cats
								$makdir = @mkdir(JPATH_SITE."/downloads/".JText::_('JLIST_SAMPLE_DATA_CAT_FOLDER_ROOT'), 0777);
								$makdir = @mkdir(JPATH_SITE."/downloads/".JText::_('JLIST_SAMPLE_DATA_CAT_FOLDER_ROOT').'/'.JText::_('JLIST_SAMPLE_DATA_CAT_FOLDER_SUB'), 0777);
								// cat erstellen in db
								if ($makdir) {
									...
									$dest_path = JPATH_SITE.'/downloads/'.JText::_('JLIST_SAMPLE_DATA_CAT_FOLDER_ROOT').'/'.JText::_('JLIST_SAMPLE_DATA_CAT_FOLDER_SUB').'/mod_jdownloads_top_1.5.zip';
...
<a href="index2.php?option=com_jdownloads"><big><strong><?php echo JText::_('JLIST_INSTALL_16'); ?></strong></big></a><br />
<br />
</center>
				<?php
			}
...
// und l�scht abscchliessend das $source dir
function moveDirs($source, $dest, $recursive = true, $message) {
	...
	@closedir($handle);
	// $source l�schen wenn KEIN error
	if (!$error) {
...
// delete_dir_and_allfiles - rekursiv l�schen
// Rueckgabewerte:
//    0 - ok
//   -1 - kein Verzeichnis
